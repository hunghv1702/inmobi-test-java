...
public class UserController implements UserApi {
    ...
    @Override
    public ResponseEntity<CurrentUserApiResponse> buyTurns() {
        Long userId = SecurityUtils.getCurrentUserId();
        log.info("Attempting to buy turns directly: userId={}", userId);
        CurrentUserResponse response = userService.addTurns(userId, 5);
        log.info("Successfully bought turns directly: userId={}", userId);
        return ResponseEntity.ok()
            .cacheControl(CacheControl.noStore())
            .body(apiResponseMapper.toSuccessResponse(response));
    }
